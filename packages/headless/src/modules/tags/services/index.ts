...
export class TagsService {
  ...
  async updateTag(tagId: string, data: UpdateTagRequest): Promise<UpdateTagResponse> {
    const doesTagNameMatch = data.name && await this.tagsRepo.getTag({ name: data.name, id: { $ne: tagId } });
    if (doesTagNameMatch) return this.helper.serviceResponse.errorResponse(UpdateTagErrorMessages.TAG_NAME_EXISTS, null, HttpStatus.BAD_REQUEST);

    const updatedTag = await this.tagsRepo.updateTag({ id: tagId }, data);
    if (!updatedTag) return this.helper.serviceResponse.errorResponse(UpdateTagErrorMessages.CAN_NOT_UPDATE_TAG, null, HttpStatus.BAD_REQUEST);
    return this.helper.serviceResponse.successResponse(updatedTag, HttpStatus.OK);
  }
}