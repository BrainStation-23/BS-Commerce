...
export class OrderCustomerService {
  ...
  async reOrder(userId: string, body: ReOrderQuery): Promise<ReOrderResponse>{
    ...
    if(availableProductIds.length === 0){
          const response = { products: availableProductIds, reDirectHome: true, message: ErrorMessageReOrder.ALL_ITEMS_INVALID};
          ...
          if( availableProductIds.length < productIds.length && ignoreInvalidItems === false){
              return {code: 200, data: {products: unavailableProducts, message: ErrorMessageReOrder.INVALID_ITEMS } };  
          }
    ...
    if (cart.items.length !== 0){
        if(overWriteCart === true){
            ...
            if(!deleteCart) return {error: ErrorMessageReOrder.CANNOT_CLEAR_CART, errors: null, code: HttpStatus.INTERNAL_SERVER_ERROR};

        }
        else return {code: 200, data: {message: ErrorMessageReOrder.OVERWRITE_CART}};
    }
    ...
    else return {error: ErrorMessageReOrder.CANNOT_ADD_ITEMS, errors: null, code: HttpStatus.INTERNAL_SERVER_ERROR};
  }
