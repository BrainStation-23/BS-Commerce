...
export class OrderAdminController {
  ...
  @ApiResponse({
    type: OrderStatusEnumDto,
    description: 'Order status, payment, shipment status enums',
  })
  @Get('enums')
  async getOrderEnums(
    @Res({ passthrough: true }) res: Response,
  ): Promise<IServiceResponse<OrderStatusEnumDto>> {
  ...
  async getOrderStatistics(
    @Res({ passthrough: true }) res: Response,
  ): Promise<IServiceResponse<OrderStatDto>> {
    const { code, ...response } =
      await this.orderAdminService.getOrderStatistics();
    res.status(code);
    return response;
  }

  @ApiResponse({
    type: OrderIncompleteStatDto,
    description: 'Order incomplete statistics',
  })
  @Get('incomplete/statistics')
  async getIncompleteStatistics(
    @Res({ passthrough: true }) res: Response,
  ): Promise<IServiceResponse<OrderIncompleteStatDto>> {
    const { code, ...response } =
      await this.orderAdminService.getIncompleteStatistics();
    res.status(code);
    return response;
  }
  ...
  @Get('orderList')
  async getOrderList(
    @Query() query: GetAllOrderQueryDto,
    @Res({ passthrough: true }) res: Response,
  ) {
  ...
  async getOrderById(
    ...
    const { code, ...response } = await this.orderAdminService.getOrderById(
      orderId,
    );
  ...
  async changeStatus(
    ...
    const { code, ...response } = await this.orderAdminService.changeStatus(
      body,
    );
    res.status(code);
    return response;
