...
export class WishListService {
  ...
  async addToWishList(
    ...
    if (!doesWishListExist) {
      ...
      if (!wishList)
        return this.helper.serviceResponse.errorResponse(
          addToWishlistErrorMessage.CAN_NOT_CREATE_WISHLIST,
          null,
          HttpStatus.INTERNAL_SERVER_ERROR,
        );
      return this.helper.serviceResponse.successResponse(
        await this.wishListRepo.getWishlistProduct(wishList),
        HttpStatus.CREATED,
      );
    }

    const doesItemExist = await this.wishListRepo.doesItemExist(
      userId,
      item.productId,
    );
    if (!doesItemExist) {
      ...
      if (!wishlist)
        return this.helper.serviceResponse.errorResponse(
          addToWishlistErrorMessage.CAN_NOT_ADD_ITEM_TO_THE_WISHLIST,
          null,
    ...
    const wishList = await this.wishListRepo.incrementItemQuantity(
      userId,
      item,
    );
    if (!wishList)
      return this.helper.serviceResponse.errorResponse(
        addToWishlistErrorMessage.CAN_NOT_INCREMENT_WISHLIST_ITEM,
        null,
        HttpStatus.BAD_REQUEST,
      );
    return this.helper.serviceResponse.successResponse(
  ...
  async getUserWishlist(userId: string): Promise<getUserWishlistResponse> {
    ...
    if (!wishList)
      return this.helper.serviceResponse.errorResponse(
        getUserWishlistErrorMessage.NO_WISHLIST_FOUND,
        null,
        HttpStatus.BAD_REQUEST,
      );
    return this.helper.serviceResponse.successResponse(
  ...
  async deleteWishlist(wishlistId: string): Promise<deleteWishlistResponse> {
    ...
    if (!wishList)
      return this.helper.serviceResponse.errorResponse(
        deleteWishlistErrorMessage.CAN_NOT_DELETE_WISHLIST,
        null,
        HttpStatus.BAD_REQUEST,
      );
    return this.helper.serviceResponse.successResponse(
      { message: deleteWishlistSuccessMessage.WISHLIST_DELETED_SUCCESSFUL },
      HttpStatus.OK,
    );
  }
  ...
  async updateWishlistItem(
    item: WishlistItem,
    userId: string,
  ): Promise<updateWishlistItemResponse> {
    ...
    const wishList = await this.wishListRepo.deleteWishlistItem(
      userId,
      item.productId,
    );
    if (!wishList)
  ...
  async deleteWishlistItem(
    ...
    if (!wishList)
      return this.helper.serviceResponse.errorResponse(
        deleteWishlistItemErrorMessage.CAN_NOT_DELETE_WISHLIST_ITEM,
        null,
        HttpStatus.BAD_REQUEST,
      );
    return this.helper.serviceResponse.successResponse(
  ...
  async deleteAllWishlistItems(
    userId: string,
  ): Promise<deleteAllWishlistItemsResponse> {
    const wishList = await this.wishListRepo.deleteAllWishlistItems(userId);
    if (!wishList)
      return this.helper.serviceResponse.errorResponse(
        deleteAllWishlistItemsErrorMessage.CAN_NOT_DELETE_ALL_WISHLIST_ITEMS,
        null,
        HttpStatus.BAD_REQUEST,
      );
    return this.helper.serviceResponse.successResponse(
      {
        message:
          deleteAllWishlistItemsSuccessMessage.WISHLIST_ITEMS_DELETED_SUCCESSFUL,
      },
      HttpStatus.OK,
    );
  }
