...
describe('Initializing... Admin Auth controller testing', () => {
  ...
  describe('POST /auth/signup [passing valid req body]', () => {
    it(
      'should return 201 and and return created admin response data',
      async () => {
      ...
      async () => {
        return await request(app.getHttpServer())
          ...
          .expect((res) => {
            ...
            expect(res.body.errors).toEqual({
              password: [
                'Password is too short. Minimal length is 6 characters',
  ...
  describe('POST /auth/signin', () => {
    ...
    describe('authentication', () => {
      it(
        'authenticates user with valid credentials and provides a jwt token',
        async () => {
      ...
      it(
        'fails to authenticate user with an incorrect password/username',
        async () => {
          return await request(app.getHttpServer())
            .post('/auth/signin')
      ...
      it(
        'fails to authenticate user without passing username',
        async () => {
          return await request(app.getHttpServer())
            .post('/auth/signin')
      ...
      it(
        'fails to authenticate user without passing password',
        async () => {
          return await request(app.getHttpServer())
            .post('/auth/signin')
            ...
            .expect((res) => {
              expect(res.statusCode).toBe(422);
              expect(res.body.errors).toEqual({
                password: ['INVALID_CREDENTIALS', 'password must be a string'],
              });
            });
        },
        TestTimeout,
