...
import { CreateTagRequest, CreateTagRequestWithoutName } from './tags.predefined.data';
        ...
        it('should error message with 422 bad request(Unprocessable Entity)', async () => {
            return await request(app.getHttpServer())
                ...
                .expect((res) => {
                    ...
                    expect(res.body.errors).toEqual({ name: ["name should not be empty", "name must be a string"] });
                });
        }, timeout);
    });
});