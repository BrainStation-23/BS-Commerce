...
async function run() {
    ...
    try {
        ...
        const collectionsContainingDocs = await getCollectionsContainingDocs(modelFiles);
...
const getSrcFiles = () => {
    ...
    srcDirectory.forEach(fileOrSubDirectory => {
        ...
        if (fs.statSync(modulePath).isDirectory()) {
            ...
            srcFiles.push(...moduleFiles.map((filePath) => `${fileOrSubDirectory}/${filePath}`));
        }
    });
    return srcFiles;
}

const getCollectionsContainingDocs = async (modelFiles: string[]): Promise<string[]> => {
    try {

        const collectionsContainingDocs = [];
        for (const file of modelFiles) {
            const { default: model }: { default: Model<any> } = await import(`./${file}`);
...
const getConfirmation = async (collections: string[]): Promise<boolean> => {
    try {
        ...
        return new Promise((resolve, reject) => {
            ...
            console.log('All the previous data of thse collections will be lost forever:');
